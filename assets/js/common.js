...
// 项目名称
var projectName = 'cangzhou';
//上传文件路径
var file_path = 'D:\\web\\前端\\upload';
var address = '';
if (projectName == "chongjiao") { //重交地址
	address = 'www.zne-ais.com';
	file_path = 'D:\\web\\前端\\upload';
} else if (projectName == "guangzhou") { //广州地址
	address = 'www.zgdrkj.cn';
	file_path = 'D:\\web\\前端\\upload'
} else if (projectName == "shanghai") { //上海地址
	address = 'www.zhilidaolu.com';
	file_path = 'E:\\web\\前端\\upload'
} else if (projectName == "cangzhou") { //沧州地址
	address = 'test.zgdrkj.cn';
	file_path = 'D:\\web\\前端\\upload'
}
//图片地址，前端本地地址
var imgUrl = 'http://' + address + ':8082/';
//后台接口地址
var url = 'http://' + address + ':8081/CjManager/';
...
//这里是sockect接口地址
var socket_url = 'wss://' + address + '/';
if(projectName == "cangzhou"){
	serverUrl=url+":8443";
	api_url= 'https://' + address + ':8443/';
	socket_url = 'wss://' + address + ':8443/';
}
//视频动态url
...
function setVideoFlvJsPlayer(playerid, url, playerwidth, playerheight) {
	var Hls = window.Hls;
	$("#" + playerid).css("width", playerwidth);
	$("#" + playerid).css("height", playerheight);
	$("#" + playerid).css("background", "#000000");
	//var videid=$("#"+playerid).attr("id");
	var vedioobj = document.getElementById(playerid);
	if (Hls.isSupported()) {
		var hls = new Hls()
		hls.loadSource(url);
