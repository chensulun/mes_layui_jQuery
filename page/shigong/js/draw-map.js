...
window.draw = (function () {
    ...
    */
    ...
    DrawCarByGPS.prototype.timesColorInit = function () {
        ...
        for (var o = 0; o < 100; o++)
            e.fillRect(t.x + o, t.y, 20 - o, 1);
        var r = e.getImageData(0, 0, 20, 1)
            , n = [];
    ...
    */
    ...
    DrawCarByGPS.prototype.drawText = function (e, t, o, r, n, i, s, task) {
        if (!!task) {
            console.log('绘制桩号', t, o, r, n, i, s)
        } else {
            console.log('其他绘制', t, o, r, n, i, s)
        }
        e.font = r,
            e.fillStyle = n,
            0 != i && (e.lineWidth = i,
    ...
    DrawCarByGPS.prototype.getCarDataColor2 = function (data, dataIndex) {
        ...
        if (dataIndex == 0) {
            ...
            if (data[dataIndex] < 100) {
                ...
                if (data[dataIndex] < 110) {
                    return "#0e9a7e";
                } else {
                    if (data[dataIndex] < 120) {
                        return "#0b7b65";
                    } else {
                        if (data[dataIndex] < 130) {
                            return "#07604f";
                        } else {
                            if (data[dataIndex] < 140) {
                                return "#044437";
                            } else {
                                if (data[dataIndex] < 150) {
                                    return "#e8b26f";
                                } else {
                                    if (data[dataIndex] < 160) {
                                        return "#d89542";
                                    } else {
                                        if (data[dataIndex] < 170) {
                                            return "#e8850b";
                                        } else {
                                            if (data[dataIndex] < 180) {
                                                return "#e45d29";
                                            } else {
                                                if (data[dataIndex] > 180) {
                                                    return "#f0550a";
    ...
    */
    ...
    DrawCarByGPS.prototype.drawCarDataNew = function () {
        ...
        2 == T.dataIndex && this.clear(this.canvasCalc, "#fff");
        var e = !0
            , t = null;
        T.drawCount = 0;
        let drawTagTotal = -1 // 桩号绘制统计
        const drawTagQueue = [] // 桩号绘制队列

        for (var o = 0, r = T.carGpsList; o < r.length; o++) {
            var n = r[o]
                , i = T.machineDic[n.id];

            if (window.type == 1 || window.type == 2) {
                const map = {
                    1: '摊铺机',
                    2: '压路机'
                }
                if (n.type != map[window.type]) {
                    continue
                }
            }
            ...
            if (void 0 !== i) {
                ...
                if (null != s) {
                    ...
                    console.log('testtest', n, drawTagTotal)
                    // 绘制桩号
                    if (n.type === '摊铺机') {
                        drawTagTotal++
                        // if (drawTagTotal % 25 === 0) {
                        // drawTagQueue.push({ s, n })
                        // }
                        if (drawTagTotal % 25 === 0) {
                            // let temp = drawTagQueue.find(v => `${v.stakemark}+${parseInt(v.evennumber)}` == `${n.stakemark}+${parseInt(n.evennumber)}`)
                            // if (!temp) {
                            drawTagQueue.push({ s, n })
                            // }
                        }
                        // n.stakemark}+${parseInt(n.evennumber
                    }
                        ...
                        if (false)
                            ...
                            if (i.show) {
                                if (until.intersectBox(this.boundingBox, s.p1, s.p2, g.p1, g.p2)) {
                                    ...
                                    } else if (3 == T.dataIndex) {
                                        var y = this.drawGpsRectNew(this.canvasCalc, s, g, p, n);

                                    } else {
                                        this.drawGpsRectNew(this.canvasMain, s, g, p, n);
                                    }
                                    i.gpsPoint = g
                                } else
                                    i.gpsPoint = g;
        ...
        // 绘制桩号
        drawTagQueue.forEach(v => {
            this.drawTag(v.s, v.n)
        })
    ...
    DrawCarByGPS.prototype.drawCar = function () {
        ...
        var renderMachine = function (machine, draw) {
            ...
            if (window.type == 1 || window.type == 2) {
                if (machine.type != window.type) {
                    return
                }
            }
            console.log('drawCar', machine)
    ...
    //绘制桩号
    DrawCarByGPS.prototype.drawTag = function (s, n) {
        // canvas.fillText(n.stakemark, s.x, s.y )
        ...
        var lxy = new mapToObjectXY(s.p2.x, s.p2.y);
        var l = this.WorldtoMap(lxy);

        this.drawText(this.canvasMain, `${n.stakemark}+${parseInt(n.evennumber)}`, { x: l.x + 20, y: l.y }, '14px 黑体', '#f00', 4, "#fff", '绘制桩号')

        // console.log('绘制桩号', n.stakemark, {x: s.p.x - 100, y: s.p.y - 100}, '12px 黑体', '#f00', 4, "#fff")
...
window.T = {
    ...
    init: function () {
        var timesOrginColorList = JSON.parse(`[{"colorVal":[186,244,249]},{"colorVal":[41,213,20]},{"colorVal":[255,226,198]},{"colorVal":[255,173,152]},{"colorVal":[255,173,152]},{"colorVal":[254,158,134]},{"colorVal":[227,141,119]},{"colorVal":[213,115,90]},{"colorVal":[255,68,61]},{"colorVal":[255,15,15]},{"colorVal":[199,41,0]}]`)
...
// let carGpsList = []
//         if (window.type == 1) {
//             carGpsList = T.carGpsList.filter(v=> {
//                 console.log(v.type)
//                 v.type === '摊铺机'
//             })
//         } else {
...
//         console.log('carGpsList', carGpsList)
